...
params = {}
params['mode'] = 'dc' # 'Path Following': 'pf', 'Static Obstalces': 'sc', "Dynamic Obstalces": 'dc'
Fmag = np.inf
if params['mode'] == 'pf':
...
params['rew_scale'] = tune.grid_search([1.0, 1.5, 2.0, 2.5])
...
epsilon_timesteps = 1350000
config = DQNConfig()
config.lr = 0.0005
config.lr_schedule = None
config.num_steps_sampled_before_learning_starts = 100000  # Learning Starts
...
config.replay_buffer_config = replay_buffer_config
...
config.hiddens = [128, 128] #tune.grid_search([[128, 128], [256, 256]])
config.double_q = False

config.target_network_update_freq = 1
config.n_step = 1
config.td_error_loss_fn = 'huber'
config.env = Multi_Agent_Env
config.env_config = params
config.num_rollout_workers = 23
config.num_gpus = 1
